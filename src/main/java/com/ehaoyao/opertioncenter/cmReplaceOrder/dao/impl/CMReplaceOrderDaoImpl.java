package com.ehaoyao.opertioncenter.cmReplaceOrder.dao.impl;
...
@Repository
public class CMReplaceOrderDaoImpl implements ICMReplaceOrderDao {
	...
	// 运营中心jdbcTemplate
			...
			public int getCountOrderInfos(ThirdOrderAuditVO vo)  throws Exception{
				...
				sql.append(" WHERE 1=1 and prescription_type='0' ");
			...
			public List<OrderInfo> getOrderInfos(PageModel<OrderInfo> pm, ThirdOrderAuditVO vo) throws Exception{
				StringBuffer sql = new StringBuffer();
				sql.append("SELECT ( case when audit_status='WAIT' then 1 when audit_status!='WAIT' then 2 end ) as seqno, order_id ,  order_number ,  order_flag ,  user_id ,  user_code ,  nick_name ,  user_name ,  prescription_type ,  order_status , "
						+ " audit_status ,  pay_status ,  order_type ,  pay_type ,  order_time ,  price ,  receiver ,  audit_user ,  patient_name ,  sex ,  age , "
						+ " pregnant_flag ,  decoct_flag ,  dose ,  small_pic_link ,  big_pic_link ,  payment_time ,  audit_time ,  delivery_date ,  address_detail , "
				...
				sql.append(getOrderDataSql(vo));
				sql.append(" order by seqno asc,order_time asc ");
				if (pm.getPageSize() > 0) {
