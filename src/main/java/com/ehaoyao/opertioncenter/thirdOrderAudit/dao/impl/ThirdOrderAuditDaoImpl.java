package com.ehaoyao.opertioncenter.thirdOrderAudit.dao.impl;
...
@Repository
public class ThirdOrderAuditDaoImpl implements IThirdOrderAuditDao {
	
	private static final Logger logger = Logger.getLogger(ThirdOrderAuditDaoImpl.class);
	...
	public List<OrderMainInfo> getOrderInfos(PageModel<OrderMainInfo> pm, ThirdOrderAuditVO vo) throws Exception{
		...
		logger.info("【运营中心-一级客服审核查询sql:】"+sql.toString());
		return list;
	}
	...
	private String getOrderDataSql(ThirdOrderAuditVO vo)  throws Exception{
		StringBuffer sql = new StringBuffer();
		//获取需要二级审核的渠道并作为查询条件
		ParamConfig paramConfig = this.getParamValue("operation_center_presc_audit", "second_presc_audit");
		String orderFlags = "";
		if(paramConfig!=null){
			String paramValue = paramConfig.getParamValue();
			if(paramValue!=null && paramValue.trim().length()>0){
				String[] valArr = paramValue.split(",");
		...
		if(orderFlags.length()>0){
			sql.append(" and g.order_flag in ("+orderFlags+") ");
		}
	...
	public Object getOrderInfosByOrderNumFlags(String orderNumFlags) throws Exception {
		...
		Object retnObj = null;
		String sql = "select   f.order_id ,  f.order_number ,  f.order_flag ,  f.user_id ,  f.user_code ,  f.nick_name ,  f.user_name ,  f.prescription_type ,  f.order_status ,"
				+ "  f.audit_status , f.invoice_status ,  f.pay_status ,  order_type ,  f.pay_type ,  f.order_time ,  f.price ,  f.receiver ,  f.patient_name ,  f.sex ,  f.age , "
				+ " f.pregnant_flag ,  f.decoct_flag ,  f.dose ,  f.small_pic_link ,  f.big_pic_link ,  f.payment_time ,  f.delivery_date ,  f.address_detail , "
				+ " f.mobile ,  f.telephone ,  f.province ,  f.city , f.area,  f.country ,  f.order_price ,  f.discount_amount ,  f.instructions ,  f.remark , "
				+ " f.fee_type ,  f.last_time ,  f.create_time "
				+ " from order_info f where 1=1 ";
		if(orderNumFlags!=null && orderNumFlags.length()>0){
			orderNumArr = orderNumFlags.split(",");
			if(orderNumArr!=null){
				if(orderNumArr.length==1){
					sql += " and CONCAT(f.order_number,f.order_flag)='"+orderNumFlags+"'";
				}
				if(orderNumArr.length>1){
					sql += " and CONCAT(f.order_number,f.order_flag) in ('"+orderNumFlags.replace(",", "','").replace(" ", "")+"') ";
				}
			}
	...
	public int[] updateOrderInfoBatch(final List<OrderInfo> orderInfoList) throws Exception{
		String sql = " update order_info set user_id=?, user_code=?, nick_name=?, user_name=?, prescription_type=?, order_status=?, "
				+ " audit_status=?, invoice_status=?, pay_status=?, order_type=?, pay_type=?, order_time=?, price=?, receiver=?, patient_name=?,"
		...
		return jdbcTemplate.batchUpdate(sql, new BatchPreparedStatementSetter() {
			...
			public void setValues(PreparedStatement ps, int i) throws SQLException {
				...
				ps.setString(j++, orderInfo.getInvoiceStatus());
	...
	public Object addOrderAuditLogBatch(final List<OrderAuditLog> orderAuditLogList) throws Exception {
		String sql = "INSERT INTO order_audit_log "
				+ "(order_number, order_flag, kf_account, audit_user, audit_status, audit_description, create_time,remark,reject_type) VALUES"
				+ " (?,?,?,?,?,?,?,?,?) ";
		return jdbcTemplate.batchUpdate(sql, new BatchPreparedStatementSetter() {
			...
			public void setValues(PreparedStatement ps, int i) throws SQLException {
				...
				ps.setString(j++, orderAuditLog.getRemark());
	...
	@Override
	public Object addInvoiceInfoBatch(final List<InvoiceInfo> invoiceInfoList) throws Exception {
		String sql = "INSERT INTO invoice_info "
				+ "(order_number, order_flag, kf_account, invoice_status, invoice_type, invoice_title, invoice_content, create_time,remark) VALUES"
				+ " (?,?,?,?,?,?,?,?,?) ";
		return jdbcTemplate.batchUpdate(sql, new BatchPreparedStatementSetter() {
			...
			public void setValues(PreparedStatement ps, int i) throws SQLException {
				...
				int j=1;
				ps.setString(j++, invoiceInfo.getOrderNumber());
				ps.setString(j++, invoiceInfo.getOrderFlag());
				ps.setString(j++, invoiceInfo.getKfAccount());
				ps.setString(j++, invoiceInfo.getInvoiceStatus());
				ps.setString(j++, invoiceInfo.getInvoiceType());
				ps.setString(j++, invoiceInfo.getInvoiceTitle());
				ps.setString(j++, invoiceInfo.getInvoiceContent());
				ps.setString(j++, invoiceInfo.getCreateTime());
				ps.setString(j++, invoiceInfo.getRemark());
			}
			
			@Override
			public int getBatchSize() {
				return invoiceInfoList.size();
			}
		});
	}
	...
	@Override
	public ParamConfig getParamValue(String paramType,String paramName)  throws Exception{
		StringBuffer sql = new StringBuffer();
		sql.append(" SELECT * from param_config where  param_status='1' ");
		if(paramType!=null && paramType.trim().length()>0){
			sql.append(" and param_type='"+paramType.trim()+"' ");
		}
		if(paramName!=null && paramName.trim().length()>0){
			sql.append(" and param_name='"+paramName.trim()+"' ");
		}
		return jdbcTemplate.queryForObject(sql.toString(), BeanPropertyRowMapper.newInstance(ParamConfig.class));
	}
