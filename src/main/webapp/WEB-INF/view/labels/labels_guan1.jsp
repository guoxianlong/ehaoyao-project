 ...
 //设置页面显示的条数
	...
	function findNextb(){
	   ...
	   function chang(){
	        ...
	        document.form.action = "labels.do?method=guan1&labelName="+name+"&id="+id;
			document.form.submit();
	    }
</script>
<style>
...
<body onload="result();">
	...
	<div class="right_h"><h3>标签分类管理三级（<span><c:if test="${labelsClass.labelName == null}">三级所有信息</c:if>${labelsClass.labelName}</span>）</h3></div>
	...
	<form action="labels.do?method=guan1&id=${parentId}" name="form" method="post">
      <div class="right_inline" style="padding-top:15px;">
		...
		<sec:authorize ifAnyGranted="/labels.do?method=addNextb,/labels.do" >
			<a class="btn btn-primary btn-xs" href="<%=request.getContextPath()%>/labels.do?method=addNextb&pId=${parentId}">增加</a>
		</sec:authorize>
			<a class="btn btn-success btn-xs" href="<%=request.getContextPath() %>/labels.do?method=guan&id=${hahaha}">返回</a>
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	...
	<table width="100%" border="0" class="table table-bordered">
				...
				<c:forEach items="${yhuserList}" var="item">
					<tr>
						...
						<td>
							<sec:authorize ifAnyGranted="/labels.do?method=addChildc,/labels.do" >
								<a class="btn btn-primary btn-xs" href="<%=request.getContextPath()%>/labels.do?method=addChildc&id=${item.id}">增加子节点</a>
							</sec:authorize>
							<sec:authorize ifAnyGranted="/labels.do?method=delb,/labels.do" >
								<a class="btn btn-info btn-xs" href="<%=request.getContextPath() %>/labels.do?method=delb&id=${item.id}&url=guan&parentId=${parentId}" onclick="if(confirm('确定删除?')==false)return false;">删除</a>
							</sec:authorize>
							<sec:authorize ifAnyGranted="/labels.do?method=editSan,/labels.do" >
								<a class="btn btn-success btn-xs" href="<%=request.getContextPath() %>/labels.do?method=editSan&id=${item.id}">修改</a>
							</sec:authorize>
							<sec:authorize ifAnyGranted="/labels.do?method=guan2,/labels.do" >
								<a class="btn btn-warning btn-xs" href="<%=request.getContextPath() %>/labels.do?method=guan2&id=${item.id}">管理</a>
							</sec:authorize>
							<sec:authorize ifAnyGranted="/labels.do?method=delForce,/labels.do" >
						    	<a class="btn btn-info btn-xs" href="<%=request.getContextPath() %>/labels.do?method=delForce&id=${item.id}&url=guan1&parentId=${parentId}" onclick="if(confirm('确定要强制删除?')==false)return false;">强制删除</a>
						    </sec:authorize>
						</td>
					</tr>
				</c:forEach>
