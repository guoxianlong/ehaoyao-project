...
<script type="text/javascript">
	...
	function checkName( flag ){
		...
		$.ajax( {   
		     type : "POST",   
		     url : "<%=request.getContextPath()%>/role.do?method=checkName&name=" + name,   
		     dataType: "json",   
		     async : flag,
...
<body>
	<form id="roleForm" action="<%=request.getContextPath()%>/role.do?method=save"
		method="post">
		<div class="right_box" id="ll"></div>
		<div class="right_h right_bg">创建角色信息</div>
		...
		<div id="namediv" class="right_inline">
			...
			<div class="right_input" style="width:300px;">
				<input id="name" name="name" value="ROLE_" onfocus="removeCss('namediv')" data-bvalidator="minlength[7],maxlength[50],required" onblur="checkName(true)"  type="text" placeholder="ROLE_ADMIN" class="form-control input-sm">
			</div>
		</div>
		...
		<div id="remarkdiv" class="right_inline">
			...
			<div class="right_input" style="width:300px;">
				<input id="remark" name="remark" data-bvalidator="maxlength[100],required" onblur="checkReamrk()" placeholder="管理员角色" class="form-control input-sm" />
			</div>
		</div>
