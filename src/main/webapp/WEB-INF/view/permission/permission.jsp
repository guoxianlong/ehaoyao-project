...
<script type="text/javascript">
    ...
    function deletePerm(obj){
		if( confirm("是否删除? ")){
			$.ajax( {   
			     type : "POST",   
			     url : "<%=request.getContextPath()%>/permission.do?method=del&id=" + obj.id,   
			     dataType: "json",   
			     success : function(data) {   
			    	 if( data.result == "true" ){
			    		 window.location.href="<%=request.getContextPath()%>/permission.do?method=permission";
...
<body>
<form action="permission.do?method=permission" name="form" method="post">
<div class="panel panel-primary">
  ...
  <div class="panel-body">
  	<input type="hidden" id="pageno" name="pageno" value="${pageno}" /> <input
		...
		<sec:authorize ifAnyGranted="/permission.do?method=add,/permission.do" >
			<a class="btn btn-primary btn-xs"	href="<%=request.getContextPath()%>/permission.do?method=add">添加资源</a>
		</sec:authorize>
		</div>
		...
		<table class="table table-bordered table-hover">
			...
			<c:forEach items="${permissionList}" var="permission">
				<tr>
					...
					<td>
					<sec:authorize ifAnyGranted="/permission.do?method=del,/permission.do" >
						<a id="${permission.id }" class="btn btn-info btn-xs"
							onclick='deletePerm(this)'
							href="javascript:void(0)">删除</a>
					</sec:authorize>
					<sec:authorize ifAnyGranted="/permission.do?method=data,/permission.do" >
						<a class="btn btn-success btn-xs"
							href="<%=request.getContextPath() %>/permission.do?method=data&id=${permission.id }">修改</a>
					</sec:authorize>
					</td>
				</tr>
			</c:forEach>
