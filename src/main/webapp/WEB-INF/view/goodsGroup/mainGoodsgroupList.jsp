...
<script language="javascript">
  ...
  //搜索
    function chang(ba){
    	...
    	document.form.action = "goodsGroup.do?method=load&ba="+ba;
		document.form.submit();
    }
  //高级搜索
    function gaojichang(ba){
      //var seachGoodsType=document.getElementById("seachGoodsType").value;//商品类型
      //var seachGroupType=document.getElementById("seachGroupType").value;//套餐类型
      //var seachSalesStatus=document.getElementById("seachSalesStatus").value;//是否可销售
      //var seachGroupNamet=document.getElementById("seachGroupNamet").value;//套餐名包含
	 // document.form.action = "goodsGroup.do?seachGoodsType="+seachGoodsType+"&seachGroupType="+seachGroupType+"&seachSalesStatus="+seachSalesStatus+"&seachGroupNamet="+seachGroupNamet;
   ...
   $.blockUI({ message: "处理中，请稍候...", css: {color:'#fff',border:'3px solid #aaa',backgroundColor:'#CC3300'}});
  		$.ajax({
    			...
    			success: function(data){
    	          $.each(data,function(i,result){  
	            	alert(result);
	            	document.form.action = "goodsp.do?method=mainGoodsSetMeal&id="+goodsId;
	                document.form.submit();
	             }); 
    			}
    		});
...
<body>
<form action="goodsp.do?method=mainGoodsSetMeal" name="form" method="post">
