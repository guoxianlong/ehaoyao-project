...
function check() {
	...
	var classLevel = document.form.attLevel.value.replace(/\s+/g, "");
...
<body>
<form name="form" method="post" action="<%=request.getContextPath()%>/data.do?method=update">
	<div class="right_box" id="ll">
	<div class="right_h right_bg" >数据字典信息修改</div>
		<div class="right_box_l">
			<div class="right_input" style="width:300px;"><input id="id" name="id" type="hidden" value="${dataClass.id}" class="form-control input-sm"></div>
			
			<div class="right_inline">
				<div class="right_title">编码</div>
				<div class="right_input" style="width:300px;"><input id="attCode" name="attCode" type="text" value="${dataClass.attCode}" class="form-control input-sm" onblur="checkClassCode(this.value)"></div>
			</div>
			
			<div class="right_inline">
				<div class="right_title">层级</div>
				<div class="right_input" style="width:300px;"><input id="attLevel" name="attLevel" type="text" value="${dataClass.attLevel}" class="form-control input-sm" readonly="readonly"></div>
			</div>
			
			
			<div class="right_inline">
				<div class="right_title">名称</div>
				<div class="right_input" style="width:300px;"><input id="attName" name="attName" type="text" value="${dataClass.attName}" class="form-control input-sm"></div>
			</div>
			...
			<div class="right_inline">
				...
				<div class="right_input" style="width:300px;">
					...
					<c:choose>
						...
						<c:when test="${dataClass.attLevel==1}">
						...
						<c:otherwise>
							<select name="parentId" class="form-control input-sm">
								<c:forEach items="${list}" var="goods">
									<c:choose>
										<c:when test="${goods.id == dataClass.parentId}">
												<option selected="selected" value="${goods.id}">${goods.attName}</option>
										</c:when>
...
function checkClassCode(val){
	...
	$.ajax({ 
		type: "POST",
		url: "<%=request.getContextPath()%>/data.do?method=cc&id="+id,
		data: "name="+val,
