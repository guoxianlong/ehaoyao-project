...
<script language="javascript">
  ...
  //搜索
    function chang(ba){
    	...
    	document.form.action = "goodsGroup.do?method=goodsGroupCopyList&ba="+ba+"&seachCheckbo="+seachCheckbo;
		document.form.submit();
    }
  //高级搜索
...
//复选框审核
  function shengheall(){
  	...
  	if(confirm("确定要审核选中的信息吗？")){
  		form.action="goodsGroup.do?method=alltcfbqc";
  		form.submit();
  	}else{
  		return false;
  	}
   ...
   function delGoodsGroupCopy(id){
   if(confirm("确定删除套餐副本吗？")){
  $.blockUI({ message: "处理中，请稍候...", css: {color:'#fff',border:'3px solid #aaa',backgroundColor:'#CC3300'}});
  $.ajax({
    			type : "POST",  
    			dataType: "json",//返回json格式的数据
    			url:"<%=request.getContextPath()%>/goodsGroup.do?method=delGoodsGroupCopy&id="+id,
  ...
  //批量套餐副本上架
  function taocanfbplshangjia(){
  	if(chkCheckBoxChs('checkgroup')== false){
  		alert('请至少选择一项！');
  		return;
  	}		
  	if(confirm("确定要上架选中的信息吗？")){
    ...
    for(var i=0;i<goodcheck.length;i++){
        ...
        $.ajax({
    			type : "POST",  
    			dataType: "json",//返回json格式的数据
    			url:"<%=request.getContextPath()%>/goodsGroup.do?method=taocanfbplshangjia&gcheck="+gcheck,
    			success: function(data){
   ...
   //批量套餐副本下架
  function taocanfbplxiajia(){
  	if(chkCheckBoxChs('checkgroup')== false){
  		alert('请至少选择一项！');
  		return;
  	}		
  	if(confirm("确定要下架选中的信息吗？")){
  	var goodcheck=document.getElementsByName('checkgroup');
    var gcheck="";
    for(var i=0;i<goodcheck.length;i++){
            if(goodcheck[i].checked==true){
        ...
        $.ajax({
    			type : "POST",  
    			dataType: "json",//返回json格式的数据
    			url:"<%=request.getContextPath()%>/goodsGroup.do?method=taocanfbplxiajia&gcheck="+gcheck,
    			success: function(data){
  ...
  //套餐副本上架
  function taocanfbshangjia(id){
  form.action="goodsGroup.do?method=taocanfbshangjia&id="+id;
  form.submit();
  }
  //套餐副本下架
  function taocanfbxiajia(id){
  form.action="goodsGroup.do?method=taocanfbxiajia&id="+id;
  form.submit();
  }
...
//前端商品点击第一级时出发的事件
  ...
  function tomodify(id,goodsId){
	  document.form.action = "goodsGroup.do?method=tcfbupdate&id="+id+"&maingoodsid="+goodsId;
	  document.form.submit();
  }
  //同步
  function tongbu(id){
  ...
  $.ajax({
    			type : "POST",  
    			dataType: "json",//返回json格式的数据
    			url:"<%=request.getContextPath()%>/goodsGroup.do?method=tongbu&id="+id,
    			success: function(data){
...
//图片跳转
  function totupian(id){
	  document.form.action = "goodsImage.do?method=tcfbload&id="+id+"&wtype=5";
	  document.form.submit();
  }
  //审核跳转
  function toshenghe(id,goodsId){
	  document.form.action = "goodsGroup.do?method=toqifb&id="+id+"&maingoodsid="+goodsId;
	  document.form.submit();
  }
  function zhejhidden(){
  document.getElementById("zhijfl").style.display="none";
  ...
  function seedetailed(id,maingoodsid){
	  jQuery.ligerDialog.open({ 
			...
			slide:true,
			title:'套餐副本详情', 
			url: '<%=request.getContextPath()%>/goodsGroup.do?method=detailed&id='+id+'&maingoodsid='+maingoodsid
			
		}); 
	 // document.form.action = "goodsGroup.do?method=toupdetailed&id="+id;
	  //.form.submit();
  }
  function updateshoujia(id,jiage){
  jQuery.ligerDialog.open({ 
			isDrag:true,//是否拖动
	        showMax:true,//是否显示最大化按钮
			height: 2, 
			...
			slide:true,
			title:'修改销售价格', 
			url: '<%=request.getContextPath()%>/goodsGroup.do?method=toRetailPrice&id='+id+'&jiage='+jiage
  ...
  if(confirm("确定要拷贝吗？")){
   $.blockUI({ message: "处理中，请稍候...", css: {color:'#fff',border:'3px solid #aaa',backgroundColor:'#CC3300'}});
  		$.ajax({
    			type : "POST",  
    			dataType: "json",//返回json格式的数据
    			url:"<%=request.getContextPath()%>/goodsp.do?method=toCopyTheGoodsToSetmealCopy&id="+id+"&goodsId="+goodsId,
    			success: function(data){
    	          $.each(data,function(i,result){  
	            	alert(result);
	            	document.form.action = "goodsGroup.do?method=goodsGroupCopyList";
  ...
  jQuery.ligerDialog.open({ 
			...
			slide:true,
			title:'药店', 
			url: 'http://www.ehaoyao.com/product-'+id+'.html'
...
<body>
<form action="goodsGroup.do?method=goodsGroupCopyList" name="form" method="post">
...
<div class="panel panel-default">
  <div class="panel-heading">
    <h5 class="panel-title" style="color:#31849b;">套餐副本查询</h5>
  </div>
  <div class="panel-body">
  	  <div class="col-xs-3" style="margin-top: 5px;">
	    <label>商品编码或套餐副本名称:</label>
	  </div>
	  <div class="col-xs-2" style="text-align: left;margin-left: -80px;">
	    <input id="seachGroupName" style="width: 120%;" name="seachGroupName" type="text" value="${seachGroupName }" class="form-control input-sm"  placeholder="输入商品编码或套餐副本名称...">
	  </div>
	  <div class="col-xs-2" style="text-align: left;margin-top: 5px;margin-left: 12px;">
...
<!-- 	   <div class="right_input" >
      ...
      <div class="col-xs-3" style="margin-left: -70px;">
	  	<input id="seachGroupId" name="seachGroupId" type="text" class="form-control input-sm" placeholder="请输入套餐副本id..."   value="${seachGroupId }"/>
	  </div>
	  <div class="col-xs-3" style="margin-top: 5px;">
...
<div class="right_box" style="width: 100%;">
  ...
  <div class="right_box2"  id="productPanel" style="display:none;">
  ...
  <div class="right_input" style="width:90px; ">
    <select name="seachsalesPlatform" class="form-control input-sm" >
    <option value="">全部</option>
    <c:forEach items="${listspform }" var="item">
    <option value="${item.id }" ${seachsalesPlatform == item.id? 'selected=selected':'' }>${item.platformName }</option>
    </c:forEach>
...
<div class="right_inline">
	<button type="button" onClick="gaojichang('2')" class="btn btn-info  ">高级搜索</button>
</div>
...
<div class="right_title"></div>
 ...
 <div class="right_input" style="float:right">
	 <sec:authorize ifAnyGranted="/goodsGroup.do?method=taocanfbplshangjia,/goodsGroup.do" >
	 	<button type="button" onclick="taocanfbplshangjia()" class="btn btn-info   btn-xs margin3" >批量上架</button>
	 </sec:authorize> 
	 <sec:authorize ifAnyGranted="/goodsGroup.do?method=taocanfbplxiajia,/goodsGroup.do">
 		<button type="button" onclick="taocanfbplxiajia()" class="btn btn-info   btn-xs margin3" >批量下架</button>
 	</sec:authorize>
 </div>
...
-->
 ...
 <input type="checkbox" name="checkgroup" value="${item.id}">
    ...
    <td>${item.id }</td>
    <td><a href="javascript:void(0)" onclick="wangzhang('${item.id}')">${item.groupName }</a></td>
    ...
    <td>${item.className }(${item.classLevel }级)</td>
    
    
    <td>
     <c:if test="${item.salesStatus==1 }">
               可销售(上架)
     </c:if>
     <c:if test="${item.salesStatus==2 }">
               不可销售(下架)      
     </c:if>
    ...
    <sec:authorize ifAnyGranted="/goodsGroup.do?method=detailed,/goodsGroup.do" >
    	<button type="button" onClick="seedetailed('${item.id }','${item.goodsId }')" class="btn btn-success   btn-xs"   >查看</button>
    </sec:authorize>
    <sec:authorize ifAnyGranted="/goodsGroup.do?method=tcfbupdate,/goodsGroup.do" >
    	<button type="button" onClick="tomodify('${item.id }','${item.goodsId }')" class="btn btn-success   btn-xs"   >编辑</button>
  	</sec:authorize>
  	<sec:authorize ifAnyGranted="/goodsp.do?method=toCopyTheGoodsToSetmealCopy,/goodsp.do" >
      <button type="button" onClick="copymaintotc('${item.id }','${item.goodsId }')" class="btn btn-success   btn-xs"   >拷贝主商品</button>
     </sec:authorize>
  	 <sec:authorize ifAnyGranted="/goodsImage.do?method=tcfbload,/goodsImage.do" >
     	<button type="button" onclick="totupian('${item.id }')" class="btn btn-primary  btn-xs"  >编辑图片</button>
     </sec:authorize>
     <c:if test="${item.salesStatus==1 }">
      <sec:authorize ifAnyGranted="/goodsGroup.do?method=taocanfbxiajia,/goodsGroup.do" >
       	<button type="button" onClick="taocanfbxiajia('${item.id }')" class="btn btn-success   btn-xs"   >下架</button>
      </sec:authorize>      
     </c:if>
     <c:if test="${item.salesStatus==2 }">
       <sec:authorize ifAnyGranted="/goodsGroup.do?method=taocanfbshangjia,/goodsGroup.do" >
       		<button type="button" onClick="taocanfbshangjia('${item.id }')" class="btn btn-success   btn-xs"   >上架</button>
       </sec:authorize>      
     </c:if>
    <sec:authorize ifAnyGranted="/goodsGroup.do?method=delGoodsGroupCopy,/goodsGroup.do" >
    	 <button type="button" onClick="delGoodsGroupCopy('${item.id }')" class="btn btn-success   btn-xs"   >删除</button>
	</sec:authorize>     
    <sec:authorize ifAnyGranted="/goodsGroup.do?method=tongbu,/goodsGroup.do" >
     	<button type="button" onClick="tongbu('${item.id }')" class="btn btn-success   btn-xs"   >同步</button>
    </sec:authorize>
    <sec:authorize ifAnyGranted="/goodsGroup.do?method=toRetailPrice,/goodsGroup.do" >
    	 <button type="button" onClick="updateshoujia('${item.id }','${item.retailPrice}')" class="btn btn-success   btn-xs"   >修改销售价格</button>
   </sec:authorize>
    <!--  
      <a href="#">&nbsp;编辑图片</a>
      -->
