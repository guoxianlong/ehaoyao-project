...
<script language="javascript">
  ...
  //搜索
    function chang(ba){
    	...
    	document.form.action = "goodsGroup.do?method=goodsGroupCopyList&ba="+ba;
		document.form.submit();
    }
  //高级搜索
...
//图片跳转
  function totupian(id){
	  document.form.action = "goodsImage.do?method=tcfbload&id="+id;
	  document.form.submit();
  }
  //审核跳转
   ...
   $.blockUI({ message: "处理中，请稍候...", css: {color:'#fff',border:'3px solid #aaa',backgroundColor:'#CC3300'}});
  		$.ajax({
    			...
    			success: function(data){
    	          $.each(data,function(i,result){  
	            	alert(result);
	            	document.form.action = "goodsp.do?method=mainGroupGoodsCopyList&id="+goodsId;
	                document.form.submit();
	             }); 
    			}
    		});
...
<body>
<form action="goodsp.do?method=mainGroupGoodsCopyList" name="form" method="post">
		...
		<!--  
<div class="right_box">
<div class="right_h">套餐副本查询</div>
<div class="right_inline"  style="min-width:800px;">
  <div class="right_title">套餐名快速搜索</div>
  
  <div class="right_input" style="width:360px;"><input id="seachGroupName" name="seachGroupName" type="text" value="${seachGroupName }" class="form-control input-sm"  placeholder=" "></div>
...
<div class="right_title"></div>
 ...
 <!--   
 <button type="button" onclick="taocanfbplshangjia()" class="btn btn-info   btn-xs margin3" >批量上架</button> <button type="button" onclick="taocanfbplxiajia()" class="btn btn-info   btn-xs margin3" >批量下架</button>
-->
...
-->
    ...
    <td width="8%">套餐副本ID</td>
    <td width="8%">套餐名</td>
    <td width="11%">含商品名</td>
    <td width="12%">主商品处方类型</td>
    <td width="11%">主商品规格</td>
    <td width="16%">主商品生产厂家</td>
 ...
 <input type="checkbox" name="checkgroup" value="${item.id}">
    ...
    <td style="white-space:nowrap;text-align: left;"><button type="button" onClick="copymaintotc('${item.id }','${item.goodsId }')" class="btn btn-success   btn-xs"   >拷贝主商品</button>
