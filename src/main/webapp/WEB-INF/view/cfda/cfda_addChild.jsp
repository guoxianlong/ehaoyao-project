...
<body>
	<form method="post" name="form" action="<%=request.getContextPath()%>/cfda.do?method=save&id=${parentId}">

		<div class="right_box" id="ll">
		<div class="right_h right_bg" >质检分类子节点信息创建</div>
		<div class="right_box_l">
			...
			<div class="right_inline">
				...
				<div class="right_input" style="width:300px;"><input id="classLevel" name="classLevel" type="text" value="${cfdaClass.classLevel + 1}" class="form-control input-sm" readonly="readonly"></div>
			</div>
			
			<div class="right_inline">
				<div class="right_title">父节点</div>
				<input type = "hidden" name="parentId" value="${cfdaClass.id }" />
				<div class="right_input" style="width:300px;"><input id="parentId" name="" type="text" value="${cfdaClass.className}" class="form-control input-sm" readonly="readonly"></div>
			</div>
...
function checkClassCode(val){
	$.ajax({ 
		type: "POST",
		url: "<%=request.getContextPath()%>/cfda.do?method=cc",
		data: "name="+val,
