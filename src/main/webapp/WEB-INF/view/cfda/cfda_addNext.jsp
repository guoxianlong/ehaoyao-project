...
<body>
	<form method="post" name="form" action="<%=request.getContextPath()%>/cfda.do?method=save1">

		<div class="right_box" id="ll">
		<div class="right_h right_bg" >质检分类信息创建</div>
		<div class="right_box_l">
			...
			<div class="right_inline">
				...
				<div class="right_input" style="width:300px;"><input id="classLevel" name="classLevel" type="text" value="2" class="form-control input-sm" readonly="readonly"></div>
			</div>
			...
			<div class="right_inline">
				...
				<div class="right_input" style="width:300px;">
					<select name="parentId" class="form-control input-sm" onchange="fun(this.value)">
						<option value="${cfdaClass.id}">${cfdaClass.className}</option>
						<c:forEach items="${list}" var="goods">
							<c:if test="${cfdaClass.parentId != goods.id && cfdaClass.id != goods.id}">
...
<script type="text/javascript">
	function fun(a,b){
		var b = ${cfdaClass.id};
		if(a==b){
			document.getElementById("classLevel").value = 1 + 1;
		}else{
			document.getElementById("classLevel").value = 1 + 2;
		}
	}
