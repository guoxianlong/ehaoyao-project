...
<script language="javascript">
  ...
  //搜索
    function chang(){
    	...
    	document.form.action = "cfda.do?method=load&className="+name;
		document.form.submit();
    }
     //设置页面显示的条数
...
<body onload="result();">
	...
	<div class="panel-heading" style="height: 50px;"><h5>质检分类管理一级</h5></div>
	  <div class="panel-body">
	  	<input type="hidden" id="result" value="${result}" />
	<form action="cfda.do?method=load" name="form" method="post">
      <div class="right_inline" style="padding-top:15px;">
		<input type="hidden" id="pageno" name="pageno" value="${pageno}"/>
		<input type="hidden" id="pageSize" name="pageSize" value="${pageSize}"/>
		<div class="col-xs-10" style="margin-top: 15px;">
			<sec:authorize ifAnyGranted="/cfda.do?method=add,/cfda.do" >
				<a class="btn btn-primary btn-xs" href="<%=request.getContextPath()%>/cfda.do?method=add">增加</a>
			</sec:authorize>
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	...
	<table width="100%" border="0" class="table table-bordered">
				...
				<c:forEach items="${yhuserList}" var="item">
					<tr>
						...
						<td>系统根节点</td>
						<td>
							<sec:authorize ifAnyGranted="/cfda.do?method=addChild,/cfda.do" >
								<a class="btn btn-primary btn-xs" href="<%=request.getContextPath()%>/cfda.do?method=addChild&id=${item.id}">增加子节点</a>
							</sec:authorize>
							<sec:authorize ifAnyGranted="/cfda.do?method=del,/cfda.do" >
								<a class="btn btn-info btn-xs" href="<%=request.getContextPath() %>/cfda.do?method=del&id=${item.id}&url=cfda" onclick="if(confirm('确定删除?')==false)return false;">删除</a>
							</sec:authorize>
							<sec:authorize ifAnyGranted="/cfda.do?method=edit,/cfda.do" >
								<a class="btn btn-success btn-xs" href="<%=request.getContextPath() %>/cfda.do?method=edit&id=${item.id}">修改</a>
							</sec:authorize>
							<sec:authorize ifAnyGranted="/cfda.do?method=guan,/cfda.do" >
								<a class="btn btn-warning btn-xs" href="<%=request.getContextPath() %>/cfda.do?method=guan&id=${item.id}">管理</a>
							</sec:authorize>
						</td>
					</tr>
				</c:forEach>
	...
	<!-- <div class="right_h"><h3>质检分类管理一级</h3></div> -->
