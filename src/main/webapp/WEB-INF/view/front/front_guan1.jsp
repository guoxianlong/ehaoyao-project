 ...
 //设置页面显示的条数
	...
	function findNextb(){
	   ...
	   function chang(){
	        ...
	        document.form.action = "front.do?method=guan1&className="+name+"&id="+id;
			document.form.submit();
	    }
...
<body onload="result();">
	...
	<div class="right_h"><h3>前端分类管理三级（<span><c:if test="${frontClass.className == null}">三级所有信息</c:if>${frontClass.className}</span>）</h3></div>
	<input type="hidden" id="result" value="${result}" />
	<form action="front.do?method=guan1&id=${parentId}" name="form" method="post">
      <div class="right_inline" style="padding-top:15px;">
		...
		<div class="col-xs-10" style="margin-top: 15px;">
			<sec:authorize ifAnyGranted="/front.do?method=addNextb,/front.do" >
				<a class="btn btn-primary btn-xs" href="<%=request.getContextPath()%>/front.do?method=addNextb&pId=${parentId}">增加</a>
			</sec:authorize>
			<a class="btn btn-success btn-xs" href="<%=request.getContextPath()%>/front.do?method=guan&id=${hahaha}">返回</a>
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		...
		<table width="100%" border="0" class="table table-bordered">
				...
				<c:forEach items="${yhuserList}" var="item">
					<tr>
						...
						<td>
							<sec:authorize ifAnyGranted="/front.do?method=delb,/front.do" >
								<a class="btn btn-info btn-xs" href="<%=request.getContextPath() %>/front.do?method=delb&id=${item.id}&url=guan&parentId=${parentId}" onclick="if(confirm('确定删除?')==false)return false;">删除</a>
							</sec:authorize>
							<sec:authorize ifAnyGranted="/front.do?method=editSan,/front.do" >
								<a class="btn btn-success btn-xs" href="<%=request.getContextPath() %>/front.do?method=editSan&id=${item.id}">修改</a>
							</sec:authorize>
						</td>
					</tr>
				</c:forEach>
