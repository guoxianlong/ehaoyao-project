...
// 		document.form.action = url;
	...
	function changeStatus(status,id) {
	 	$.ajax({
    		type : "POST",
    		url : "<%=request.getContextPath()%>/doctorInHospital.do?method=updateStatusSalesRep",
    		dataType: "html",
    		data : {status:status,
    				id:id},
		...
		<div class="panel panel-primary">
			...
			<div class="panel-body">
				...
				<div class="right_inline">
					...
					<button type="button" onclick="window.location.href='<%=request.getContextPath()%>/doctorInHospital.do?method=addSalesRep'"
				...
				<table  id="tab" class="table table-bordered table-hover">
					...
					<c:forEach items="${salesRepList}" var="hospital">
						<tr>
							...
							<td>
								<c:if test="${hospital.status == 0}">否</c:if>
								<c:if test="${hospital.status == 1}">是</c:if>
							</td>
							<td>
								<button type="button" class="btn btn-danger btn-xs"  onclick="changeStatus(0,${hospital.id});">失效</button>
								<button type="button" class="btn btn-success btn-xs" onclick="changeStatus(1,${hospital.id});">恢复</button>
