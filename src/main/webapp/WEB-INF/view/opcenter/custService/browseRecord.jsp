...
function gotoPage(pageno){
	...
	$.ajax({
		type: "POST",
		url:"<%=request.getContextPath()%>/"+actionName+"?method=getBrowseRecord",
        dataType: "html",
...
<div id="cardDiv" style="min-width: 700px;">
	...
	<table id="buy_table" class="table table-bordered">
		...
		<tbody id="tb_body">
			<c:forEach items="${brls}" varStatus="status" var="br">
			<tr>
				<td>${status.count}</td>
				<td>${br.mealName }</td>
				<td>${br.visitDay}</td>
			...
			<c:if test="${brls==null || fn:length(brls)<=0}">
				<tr>
					<td colspan="4" style="height:80px;text-align: center;vertical-align: middle;font-size:14px;font-weight: bold;">暂无数据！</td>
				</tr>
			</c:if>
		</tbody>
	</table>
