...
function gotoPage(pageno){
	...
	$.ajax({
		type: "POST",
		url:"<%=request.getContextPath()%>/"+actionName+"?method=getCashInfo",
        dataType: "html",
...
<div id="cardDiv" style="min-width: 700px;">
	<form id="dataForm" name="dataForm" action="" class="form-horizontal" role="form" method="post">
		...
		<div class="right_inline">
			...
			<div style="float:right;margin-right: 60px;">
				<div class="right_input">现金账户余额：<span style="font-size: 18px;color: #FCC89A">${moneyBalance}</span></div>
			</div>
	...
	<table id="buy_table" class="table table-bordered">
		...
		<tbody id="tb_body">
			<c:forEach items="${cls}" varStatus="status" var="buy">
			<tr>
				<td>${buy.consumeTime}</td>
				<td>${buy.consumeMoney }</td>
				<td>${buy.consumeContent }</td>
				<td>${buy.consumeContent }</td>
			</tr>
			</c:forEach>
			<c:if test="${cls==null || fn:length(cls)<=0}">
				<tr>
					<td colspan="4" style="height:80px;text-align: center;vertical-align: middle;font-size:14px;font-weight: bold;">暂无消费记录！</td>
				</tr>
			</c:if>
		</tbody>
	</table>
