...
<script language="javascript">
  ...
  //搜索
    function chang(ba){
    	...
    	document.form.action = "goodsp.do?method=getGoodsList&ba="+ba;
		document.form.submit();
		}
  ...
  //高级搜索
    function gaojichang(ba){
	 ...
	 // var seachmanufacturer=document.getElementById("seachmanufacturer").value;
	  ...
	  document.form.action = "goodsp.do?method=getGoodsList&starjia="+starjia+"&endjia="+endjia+"&yijid="+yijid+"&qyijid="+qyijid+"&erj="+erj+"&sanj="+sanj+"&qerj="+qerj+"&qsanj="+qsanj+"&ba="+ba;
	  document.form.submit();
    }
  function toadd(){
...
<body >
<form action="goodsp.do?method=getGoodsList" name="form" id="form" method="post">
...
<div class="panel panel-default">
  ...
  <div class="panel-body">
    <div class="row" style="vertical-align: middle;">
	  ...
	  <div class="col-xs-4" style="margin-left: -110px;margin-top: -5px;">
	    <input type="text" id="seachgoodsName" name="seachgoodsName" value="${seachgoodsName}" class="form-control" placeholder="请输入商品编码或商品名称...">
	  </div>
...
-->
 ...
 <div class="right_input" style="float:right">
 	<sec:authorize ifAnyGranted="/goodsp.do?method=toadd,/goodsp.do" >
 		<button type="button" onclick="toadd()" class="btn btn-info btn-xs margin3 " >添加</button>
 	</sec:authorize>
 	<sec:authorize ifAnyGranted="/goodsp.do,/goodsp.do?method=plsheng" >
		<button type="button" class="btn btn-info  btn-xs margin3 "  onclick="shengheall()" >批量美编审核</button>
	</sec:authorize>
	<sec:authorize ifAnyGranted="/goodsp.do,/goodsp.do?method=allplsheng" >
		<button type="button" id="meibshenheall" name="meibshenheall" onclick="allspsctcall()" class="btn btn-info btn-xs margin3 " >全部美编审核</button>
	</sec:authorize>
	<sec:authorize ifAnyGranted="/goodsp.do,/goodsp.do?method=plshengtaocan" >
		<button type="button" class="btn btn-info btn-xs margin3 "  onclick="spsctcall()">批量生成单品套餐(限质检已审)</button>
    </sec:authorize>
	<sec:authorize ifAnyGranted="/goodsp.do,/goodsp.do?method=allsctaocan" >
    	<button type="button" id="submitBtn" name="submitBtn"  class="btn btn-info btn-xs margin3 " >全部生成单品套餐</button>
    </sec:authorize>
	<!-- 
...
-->
  ...
  <tr>
    ...
    -->
    	<sec:authorize ifAnyGranted="/goodsp.do,/goodsp.do?method=load" >
    	    <button type="button" onClick="tomodify('${item.id }','1')" class="btn btn-success   btn-xs"   >编辑</button>
    	</sec:authorize>
    	<sec:authorize ifAnyGranted="/goodsImage.do,/goodsImage.do?method=load" >
	      	<button type="button" onclick="totupian('${item.id }')" class="btn btn-primary  btn-xs"  >编辑图片</button>
		</sec:authorize>
        <c:if test="${item.editStatus==0 }">
        	<sec:authorize ifAnyGranted="/goodsp.do?method=shenghe,/goodsp.do" >
      			<button type="button" onClick="toshenghe('${item.id }')" class="btn btn-warning  btn-xs"  >审核</button>
      		</sec:authorize>
        </c:if>
        <c:if test="${item.editStatus==2 }">
        	<sec:authorize ifAnyGranted="/goodsp.do?method=shenghe,/goodsp.do" >
      			<button type="button" onClick="toshenghe('${item.id }')" class="btn btn-warning  btn-xs"  >审核</button>
      		</sec:authorize>
        </c:if>
		<c:if test="${item.examineStatus==1 }">
			<sec:authorize ifAnyGranted="/goodsp.do?method=danshengtaocan,/goodsp.do" >
        		<button type="button" onClick="danshengtaocan('${item.id }')" class="btn btn-warning   btn-xs"   >生成单品套餐</button>
        	</sec:authorize>
        </c:if>
      <!-- 
        <button type="button" onClick="delgoods('${item.id }')" class="btn btn-success   btn-xs"   >删除</button>
         -->
      </td>
  </tr>
  </c:forEach>
