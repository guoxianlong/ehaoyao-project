...
$(document).ready(function(){
	$.ajax({
		...
		url:"<%=request.getContextPath()%>/goodsp.do?method=getImageList",
		success: function(data){
			 var item;  
             $.each(data,function(i,result){  
            	
                 item = "<tr><td>"+result['id']+"</td><td>"+result['pathname']+"</td><td><img class=\"pimg\" width=66 height=50  src="+result['path']+" ></img></td><td>操作</td></tr>";  
                 $('.table').append(item);  
...
<table  border="1" class="table table-striped table-bordered table-condensed">
            <tr><th>编号</th><th>图片名称</th><th>图片路径</th><th>操作</th></tr>  
        </table>
