...
(function($){
	...
	bValidator = function(mainElement, overrideOptions, instanceName){
		...
		var options = {
			...
			errorMessages: {
				en: {
					...
					'decminlength':	  'Please enter a decimal greater than or equal to {0}.',
					'decmaxlength':	  'Please enter a decimal less than or equal to {0}.',
			...
			zh_CN: {
				...
				'equalto':    '请再次输入相同的值.',
				'differs':    '请输入一个不同的值.',
				'minlength':  '长度最小值必须{0}个字符.',
				'maxlength':  '长度最大值必须在{0}个字符.',
				'rangelength':'长度必须在{0}和{1}之间.',
				'min':        '数值必须大于或等于{0}.',
				'max':        '数值必须小于或等于{0}.',
				'decminlength':	  '小数位数必须大于或者等于 {0}位.',
				'decmaxlength':	  '小数位数必须小于或者等于 {0}位.', 
				'between':    '请输入一个数字长度必须在{0}和{1}之间.',
				'required':   '此字段为必填',
				'alpha':      '请输入字母.',
				'alphanum':   '请输入数字字母.',
				'digit':      '请输入一个整数.',
		...
		validator = {
			...
			decminlength: function(v, decminlength){	 
				var sval = new String(v.value);
				var i = sval.indexOf("."); 
				if(i>=0){
					var ssval = sval.substring(i+1); 
					return (ssval.length >= parseInt(decminlength))
				}else{
					return true;	
				}
			
			},
			decmaxlength: function(v, decmaxlength){	 
				var sval = new String(v.value);
				var i = sval.indexOf("."); 
				if(i>=0){
					var ssval = sval.substring(i+1); 
					return (ssval.length <= parseInt(decmaxlength))
				}else{
					return true;	
				}
