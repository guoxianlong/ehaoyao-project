...
$(document).ready(function() {
	...
	$("#doctorInvoiceStatusDetailPage").change(
		function(){
...
function invoiceStatusFunc(obj){
	$("#doctorInvoiceTitleDetailPage").val("");
	if(obj.value == "0"){
		$("#doctorInvoiceTypeLabel").hide();
		$("#doctorInvoiceTitleLabel").hide();
		$("#doctorInvoiceContentLabel").hide();
	}else{
		$("#doctorInvoiceTypeLabel").show();
		$("#doctorInvoiceTitleLabel").show();
		$("#doctorInvoiceContentLabel").show();
	}
	$("#doctorInvoiceTitleDetailPage").focus();
}
...
function orderDetailsPageView(orderNumber,orderFlag){
	...
	$("#doctorAuditDescriptionDetailPage").val("");
...
function getOrderDetailPage(orderNumber,orderFlag){
	$.ajax({
		...
		success: function(data){
			...
			//隐藏及灰化相关功能操作
			if(orderMainInfo.auditStatus=='PRESUCC'){
				$("#auditUserLabel").hide();
				$("#succBtn").attr("disabled",false); 
				$("#retnBtn").attr("disabled",false); 
				$("#doctorAuditDescriptionDetailPage").attr("disabled",false); 
			}else{
				$("#auditUserLabel").show();
			...
			$("#auditUserDetailPage")[0].innerHTML = (orderMainInfo.doctorAuditUser==undefined?'':orderMainInfo.doctorAuditUser);
			$("#doctorAuditDescriptionDetailPage").val(orderMainInfo.doctorAuditDescription==undefined?'':orderMainInfo.doctorAuditDescription);
			
			$("#kfInvoiceStatusDetailPage option[value="+orderMainInfo.invoiceStatus+"]").attr("selected",true);
